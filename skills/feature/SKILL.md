...
```
These are added by the GATES. Every section above stays required and none of these replaces one. They appear in the file in the order their gate runs — deliberately NOT counted or numbered: a count in prose goes stale the moment a gate is added, and nothing greps for one. This paragraph used to open "Three more sections … The seven above", which was the same defect it warns about.
```
...
Plan changes an exported SIGNATURE → count call sites by grepping the SYMBOL, not the feature's surface description; the two sets differ. Sizing Impact Analysis from the wrong set understates it, and the missed callers surface as build errors mid-implementation. **A bare name grep also OVERCOUNTS** — it matches same-named local helpers and shadowed imports, which is the opposite error and looks identical in the output. Confirm each hit resolves to the import, or make the signature change first and let the compiler count.
...
**The plan you write HERE has had no premortem.** 1.4 and 1.5 ran, or were carved out, before it existed — so a plan authored at this gate is the one nobody critiqued. Crosses 1.5's threshold → run it now, against this plan, and append `# Premortem`. Record the approach answer as `# Open Questions` in the same pass; that ask IS 1.4, arriving late.
...
- every `# Debt Found` line filed as its own DRAFT in `/features/draft/` (write-only, per ON ACTIVATION 0.7; MINIMAL, per the block), its id written back onto the line. An ABSENT section is NOT proof: `# Validation` records either the filed ids or the LITERAL phrase "no debt taken" — the check greps for exactly that, so a paraphrase goes red. Debt is usually recognised HERE, reading the diff, not at the keystroke that created it — so an empty section far more often means you forgot than that you were clean
...
- changed path is harness-registered config (agent/skill/hook definition) → apply the harness-registration rule in `self-improve` SKILL LIFECYCLE before you judge it; a not-found error right after writing proves neither broken nor working. **The RUNNING copy may not be yours.** Such a definition loads from a tree — often once, before your edit — so exercising it end-to-end from your own branch can be impossible. Verify what you can, NAME the half you could not, and schedule it for after the change lands. Never report it exercised because the text on disk is right
- validating a RULE you wrote by RUNNING it yourself tests your hand-operation, not the rule. Whatever the text tells its executor to derive (a path, a command, a value) must be derived FROM THE TEXT during the test — supply it by hand and a green run proves nothing about the step you skipped, which is exactly where the rule can be wrong. Same for a value the SYSTEM resolves (config, default, env): pass it explicitly and you test the consumer, never the resolution — and a stored value that defeats your new default lives exactly there. **And the command you RAN is the command you WRITE**: a rule paraphrased from a working test drops the flag — or, where the test used SEVERAL commands, a whole COMMAND — that made it work; the test used it, the rule reads without it, and nothing compares the two. A test that separated three cases with two commands, written up with one, ships a rule that silently collapses two of them
- changed path is PROSE a model executes (skill, workflow, rule file, prompt) → "it reads fine" is NOT validation, and neither is your own re-read. You cannot audit your own prose — you know what you meant. A FRESH model with no context does it. **READ `reference/trace.md` FIRST and brief from it** — join that pointer onto this skill's announced base directory. It owns the BAR, the CORPUS, the cap, and what to do across rounds. **A trace briefed without its BAR and CORPUS is INVALID, not merely thin: discard its findings, do not weigh them.** Do not brief from this bullet; it deliberately states no bar. Delegation unavailable (harness or policy forbids unrequested agents) → say so and ask the user to authorize the ROUNDS, not one pass
...
- do NOT make the DELIVERABLE commit here. It waits until after DONE (Step 7), and is user-opt-in. Intermediate commits already made during Step 5 are fine and stay. **New ones are MADE here too, one per fix round** — the blast-radius count below reads its diff off git, so a round that is not its own commit leaves nothing to read. Those are not the deliverable commit and the sentence above is untouched by them.
Verification fails (build/tests red) → fix the root cause, re-run. Same check fails again after a fix attempt → stop, report the failure and your diagnosis to the user, do NOT weaken the check, skip it, or keep guessing at patches. Ask before a third attempt at the same failing check. Same stop when each fix round CLOSES its named defects but opens new ones in its own blast radius: two CONSECUTIVE such rounds = not converging → report the pattern and ask, don't start a third. A round that traces CLEAN, or whose findings fall OUTSIDE the preceding fix round's diff, RESETS the count — the stop is for a divergence spiral, not for two unrelated rounds far apart. Count only the BLAST-RADIUS findings, and count them MECHANICALLY: a finding is blast-radius when its `file:line` falls inside the immediately preceding fix round's diff. **That diff has to be a real git artifact, so COMMIT EACH FIX ROUND before the next trace round runs** — batch them and there is nothing to read, and a number you assembled by hand is not a mechanical count. A round that changed NOTHING has an EMPTY diff, never the previous round's: every finding then falls outside it, which resets the count correctly. No preceding fix round at all — the FIRST trace round, which traces the implementation — → N=0, and never M of M against the implementation's own diff. Rounds batched anyway → state the diff EXPLICITLY, file and line ranges, never the words "measured by hand"; a fallback cheaper than the rule becomes the rule. Outside that diff = a part earlier rounds never probed = the check working, not divergence — **EXCEPT where the fix changed a DEFINITION** (a term, condition or state name other rules read). Then the blast radius is every rule depending on it, not the diff; those rules were probed and CORRECT before it. Count them IN, and say you widened the count and why. SHOW both numbers ("N of M findings were blast-radius"). The verdict itself — "not converging", "the check working" — is required, not forbidden; what is forbidden is stating it WITHOUT the two numbers it rests on. Read by eye this rule has already been applied two ways in one week — once counting 2 of 7 with no visible arithmetic, once firing after ONE round though this text says two. User overrides the stop → the next round runs on a NARROWER corpus (the last fix round's diff alone), never the same one again; an override that changes nothing just repeats the round that failed to converge.
...
- **The FINAL DELIVERABLE commit happens ONLY after the user moves the feature to DONE, and only if they opt in** via AskUserQuestion. Never at READY_FOR_DONE, never automatically. This binds a commit that carries a feature's work SIDEWAYS too: sweeping in another feature's uncommitted changes lands it without its own gate. Name it and get its opt-in, or don't commit. This governs the DELIVERABLE commit only — intermediate commits during the BUILD are fine, in Step 5 AND in Step 6, including the integration commits a delegating workflow needs to hand work between workers and the one-per-round commits Step 6's blast-radius count reads its diffs from. Those are still never made on the default branch: branch first.
