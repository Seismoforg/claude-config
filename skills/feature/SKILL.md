...
```
node C:/Users/emdef/.claude/skills/feature/scripts/check-features.mjs [root]
```
...
Spec already carries a `# Premortem` section → it ran; go to step 2.
...
- do NOT make the DELIVERABLE commit here. **New intermediate ones are MADE here, one per fix round** — the blast-radius count below reads its diff off git, so a round that is not its own commit leaves nothing to read
...
Same stop when each fix round CLOSES its named defects but opens new ones in its own blast radius: two CONSECUTIVE such rounds = not converging → report the pattern and ask, don't start a third. A round that traces CLEAN, or whose findings fall OUTSIDE the preceding fix round's diff, RESETS the count. Count only the BLAST-RADIUS findings, and count them MECHANICALLY: a finding is blast-radius when its `file:line` falls inside the immediately preceding fix round's diff. **That diff has to be a real git artifact, so COMMIT EACH FIX ROUND before the next trace round runs.** A round that changed NOTHING has an EMPTY diff, so every finding falls outside it, which resets the count correctly. No preceding fix round at all — the FIRST trace round — → N=0, never M of M. Rounds batched anyway → state the diff EXPLICITLY, file and line ranges, never "measured by hand". Outside that diff = the check working, not divergence — **EXCEPT where the fix changed a DEFINITION** (a term, condition or state name other rules read). Then the blast radius is every rule depending on it; count them IN, and say you widened the count and why. SHOW both numbers ("N of M findings were blast-radius"). The verdict — "not converging", "the check working" — is required; what is forbidden is stating it WITHOUT the two numbers it rests on. User overrides the stop → the next round runs on a NARROWER corpus (the last fix round's diff alone), never the same one again.
