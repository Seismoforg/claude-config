...
# Dispatch         (added during step 5 — one block per milestone: set · derived files · tier · why)
```
...
- **`# Dispatch`** is written as each milestone's decision is MADE, never afterwards. It is the only record of which model built what, and NOTHING checks that it exists — which is exactly why `reference/dispatch.md` §7 makes the timing the rule. Absent on a spec that reached step 5 is a defect nobody will see.

**`Open assumption:` lines** — a trailing list under `# Technical Plan`, one line each: what you assumed and why, for anything the spec could not settle. EVERY spec writer produces them: inline at step 1 and `grilling` (its §5). They are step 1.4's candidate list, so a spec that smooths its assumptions away disarms the gate that exists to confirm them.
...
**Before building each milestone, make the DISPATCH DECISION** — read `reference/dispatch.md`, joined onto this skill's announced base directory. Cut that milestone's tasks into file-disjoint sets, rate each set by the KIND of change it needs, and either build it here or dispatch a `dev` worker at `haiku`, `sonnet` or `opus`. Record set, derived files, tier and reason under `# Dispatch` as you decide, never afterwards. EVERY milestone gets a decision and there is no threshold — "all `inline`" is a legitimate one, and it short-circuits the rest of the procedure in one line. A spec with no milestone cut makes the decision once, over the whole task list.
...
- Fanning tasks out to parallel workers → `reference/dispatch.md` owns the cut, the coverage re-checks, the brief and the merge. Don't restate it here.
**When the work is in, run the pass over the FINISHED RESULT** — whether it was dispatched or built here. The cross-cutting skills that actually applied, plus their shell-bound checks: `coding-standards`' pre-flight on a design surface, `documentation`'s check-docs when the change touched docs, and `coding-standards/reference/design.md` §14 pass 2 (the reviewer dispatch) and pass 3 (the whole-page boxes) on a design surface. **None of these is dispatch-specific** — an inline build owes every one. A dispatched build owes them ON TOP of whatever each worker ran: your run covers what no worktree could see. Applied-but-unrun is not clean; a skill that never applied needs no run. Commit what that pass makes you fix.
