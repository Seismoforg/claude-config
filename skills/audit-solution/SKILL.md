...
- Keep the Tasks checklist current as findings are fixed — `feature` step 5 owns the rule (tick on landing; a removed or reworded task carries a reason). **Seed the mirror when you create the file**: the fast-path lands it straight in `in-progress/` and so skips step 4, which is where that instruction normally sits. One todo per Tasks item, verbatim. Of the two mechanical halves, exactly one reaches an audit remediation: `tasks-not-current` cannot — the fast-path never runs step 1.5, so there is no `# Premortem` to trigger it — but `tick-guard` does, because it reads neither. Where that guard is registered, skipping the seed costs you a blocked turn until you do it; where it is not, the prose rule is again all that binds.
