...
- **Ownership does NOT expire at DONE, and the test is not which folder the feature file sits in.** A feature owns its branch until step 7 is FINISHED with it — its branch question asked AND the answer executed — and step 7 moves the file to `done/` BEFORE invoking this skill, so "that feature is DONE" reads TRUE at exactly the moment Q3 must stay quiet. Ask whether a feature is still walking its step 7, not whether it is still pending.
...
- **Land the branch** (if Q3 chose it — or if `feature` step 7's own branch ask chose it and sent its executor straight here; that ask REPLACES Q3 on a feature-owned branch, which is why Q3 stays suppressed there, and this list is the procedure for both): runs ONCE, after EVERY commit of the split has landed — never inside the per-group loop above. A split leaves groups 2..N unstaged while group 1 commits; a merge in the middle carries them across the `git switch` and onto `<default>`, the exact thing the branch existed to prevent. Then, in order:
  ...
  6. `<default>` is pushed only if Q2 chose push. Otherwise the merge stays local and STEP 5 says so. Never push it unasked. **No Q2 exists in this run** — reached from `feature` step 7 after this skill STOPped at STEP 1 (clean tree, nothing unpushed), so STEP 3 never ran → treat that as "not chosen": leave `<default>` local and say so. Never read an absent answer as a yes.
