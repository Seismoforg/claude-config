...
- **Q3 — both states, and on LAND-ONLY it is the WHOLE round.** Keeps every precondition below, feature suppression included, but loses its commit half. Options here: "Keep the branch" / "Merge into `<default>` and delete the branch — here AND on `<remote>` if it is published there". The labels below both begin with "Commit"; asked unchanged they promise a commit this run cannot make.
  - **NO Q2 in this round → the label MUST also end "and push `<default>`".** Nothing else can then authorize publishing the merge, and STEP 4's land steps 6 and 7 both key on whether the authorizing answer NAMES that push. **No remote at all → drop that clause too.** **Q2 IS in this round → the label stays as written above**, because Q2 decides the push.
  - **In a round without Q2, "Keep the branch" comes FIRST.** The user typed a commit command on a clean tree; the run was not started to land.
...
- **The hygiene scan excluded a path → say HERE that landing cannot happen this run.** Land step 1 STOPs on anything left in the tree. Say it in the option's description.
...
- **The label carries the ACTION; the option's description carries why it is safe:** the branch is merged by the time it is deleted, and `git branch -d` refuses — and stops the list — if it is not.
...
# STEP 4 — EXECUTE
- **New branch first** (if chosen): `git switch -c feature/<kebab-name>`. The `feature/` prefix is the scheme for EVERY branch this config creates — inside a `feature` run the slug is the feature file's own, timestamp dropped. Slug already carries the prefix → don't double it (`feature/x`, never `feature/feature/x`). **A slug that merely BEGINS with `feature-` is NOT that case — keep it whole, `feature/feature-x`.** The branch slug must round-trip to the feature file's, or Q3's ownership fallback (`????????-????-<slug>.md`) matches nothing. This step owns the scheme; `feature` step 5 and `crew` step 4 point here.
- **Single commit:** stage intended paths (`git add -- <paths>`; `git add -A` ONLY after the hygiene scan found nothing to exclude), then `git commit -m "<subject>" -m "<body>"`.
- **Split:** per group in order — `git add -- <that group's paths>` then **`git commit -m "<subject>" -m "<body>" -- <that group's paths>`** — message flags kept, pathspec repeated on the commit. The `-m` is not optional: without it git opens `core.editor`, and a captured subprocess has no one to answer it. Keep groups disjoint. **The pathspec is what leaves already-staged-but-unrelated changes for their own group**: a bare `git commit` writes the WHOLE index, so anything staged before this run is swept into group 1 and group 2 then has nothing left to stage.
...
- **Land the branch** (if Q3 chose it — or if `feature` step 7's own branch ask chose it and sent its executor straight here; that ask REPLACES Q3 on a feature-owned branch, and this list is the procedure for both): runs LAST and runs ONCE — after EVERY commit of the split, and after the PR bullet ABOVE. Never inside the per-group loop: a split leaves groups 2..N unstaged while group 1 commits, and a merge in the middle carries them across the `git switch` onto `<default>`. If you ever find yourself here with a PR open on this branch, stop and report instead. Then, in order:
  1. Tree must be clean — **STEP 1's definition, all three parts.** Anything left → STOP; never merge on top of it. Staged belongs in that list: a split that ended early leaves group N's paths staged, and `git switch` carries them onto `<default>`. **A path the hygiene scan excluded is still "left".**
  2. **Read the branch's upstream NOW, before anything is deleted** — `git rev-parse --abbrev-ref <branch>@{upstream}` (or `git ls-remote --heads <remote> <branch>`). NOT Step 1's record: the push bullet above may have created it this run. And not later: step 6 needs this value, and by then step 5 has deleted the branch and its config with it.
  3. `git switch <default>`. **Check the exit code; a failure here must never fall through to the merge.** Inside a LINKED worktree this fails when `<default>` is checked out in another one — exit 128, `fatal: '<default>' is already used by worktree at '<path>'`. Any non-zero → report and STOP.
  4. `git merge --no-ff <branch>` — always a merge commit, so the branch stays visible in history. Conflict → `git merge --abort`, report, STOP. **Any other non-zero exit** — unrelated histories, a rejected hook, signature verification → report and STOP as well. Check the exit code, do not scan the output for the word "conflict". A half-merged tree blocks every later run (Step 1 STOPs on `MERGE_HEAD`).
  5. `git branch -d <branch>` — lower-case `-d`, never `-D`. It refuses, and that refusal is the signal to stop: report it, delete nothing, **and STOP the list here** — do not run 6 or 7, or step 6 would delete the REMOTE copy of the branch whose local delete just refused. **Exit 1 is not one cause; three share it:** `not fully merged`, `cannot delete branch 'X' used by worktree at '<path>'`, and `branch 'X' not found`. NAME the cause before reporting one, in this order. `git rev-parse --verify --quiet refs/heads/<branch>` prints nothing and exits non-zero when there is NO SUCH BRANCH — check that first, because the next command is empty for that case too and would read as "unmerged". It exists → `git branch --merged <default> --list <branch>` is EMPTY when genuinely unmerged, and prints a `+`-prefixed line when merged but checked out in another worktree. Stopping here leaves you on `<default>`. UNMERGED → the work is still on `<branch>`; say so and name it. The other two → the merge already landed, nothing is stranded, and on the worktree cause `git switch <branch>` is refused anyway (exit 128). Never offer a switch-back that git will reject.
  6. Step 2 found an upstream → `git push <remote> --delete <branch>`. Found none → nothing remote to delete; say so. Fails (already gone, no permission) → the local delete stands; report the failure, never force.
     **The merge is staying local → do not delete the remote branch either.** Both or neither: deleting the published branch while the merge stays local empties the remote of the work. Keep the branch, say it was kept, and say why. **Decide it off ONE thing: does the answer that AUTHORIZED this landing NAME the `<default>` push?** Q3's label does not on a run that also asked a Q2, so there the push rides on Q2 alone: Q2's commit-only option → this bullet FIRES; Q2's push option → it does not. **A Q3 label from a round with NO Q2 DOES name it**, so there Q3's own yes governs both halves. Keyed on the ABSENCE OF A Q2, never on the state name — in the overlap the run is LAND-ONLY *and* has a Q2. `feature` step 7's branch ask DOES name it, so a landing it authorized deletes the remote branch on ALL THREE of that step's paths. **Never key it on who CALLED you** — Q3 fails CLOSED on that fact, where this bullet would fail OPEN. **Cannot tell which answer authorized this landing → this bullet APPLIES; delete nothing remote.**
  7. `<default>` is pushed only if Q2 chose push — `git push`, or `git push -u <remote> HEAD` when `<default>` itself has no upstream. Otherwise the merge stays local and STEP 5 says so. Never push it unasked. **Reached from `feature` step 7's branch ask** — on ANY of that step's three paths → that ask governs instead and its yes covers this push. The pairing is ASYMMETRIC: what is forbidden is a remote delete with the merge left local; a push with nothing to delete is harmless. So: **Q3 landing that ASKED a Q2** → step 6 ran → push `<default>`; step 6 did not → leave it local. **Q3 landing with NO Q2** → its label names the push, so Q3's own yes governs. **Unless there is NO REMOTE**, where STEP 3 drops that clause: nothing was authorized because nothing can be pushed. **`feature` step 7 landing** → step 6's OUTCOME does not decide it: push `<default>` whether or not there was a remote branch to delete, because a never-published branch is what a `feature` run ordinarily leaves behind. What DOES decide it is that ask's own wording — **no remote at all, and that ask drops its push clause, so there is nothing to push.** Say so and leave it; the `<default>`-binding paragraph below governs instead.
  **`<default>` must be BOUND before step 3 — CHECK it here, never re-resolve it.** Both entries bind it upstream: Q3 needs the ahead-of-`<default>` count to be offered at all; `feature` step 7 binds it to word its own question. Not bound → resolve by STEP 1's recipe now. An executor that re-resolves unconditionally asks the user to name their default branch twice in one landing.
...
- **Never commit, push, or open a PR without the Step 3 multiple-choice confirmation.** "Other"/cancel → do nothing. **The land list's remote-branch delete IS a push**, and the confirmation that authorizes it is Q3's yes — or `feature` step 7's branch ask. Those two, and only those two. Neither given → no remote delete.
- **Never open a PR unless the user asked for one** — THE PR TRIGGER owns what counts, and a mention of the word is not a request.
- **Never open a PR from the default branch; never merge, approve, or mark a draft ready** unless explicitly asked — for a merge into `<default>`, **Q3's yes IS that explicit ask**, or `feature` step 7's own branch ask. Those two are the only ones this skill accepts. Opening ≠ landing; opening is outward-facing and not cleanly reversible. Never simulate an opened PR or report a URL you didn't get back.
