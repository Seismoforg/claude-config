...
- New addendum file (not a full skill) added under an existing skill → lives in `skills/<name>/reference/`; a check script lives in `skills/<name>/scripts/`. Never at the skill root. Does NOT get its own README/CLAUDE.md entry — only a pointer inside its parent skill's SKILL.md. Pointer style, by consumer — not by taste:
  ...
  - **Path handed to a SUBAGENT** → resolve to absolute first. It gets your CWD for nothing, and for a file you merely HAND it there is no announced base directory at all. (MEASURED 20260803: a subagent DOES get `Base directory for this skill:` for each skill it PRELOADS, with skill-dir placeholders inside that body already substituted — so a preloaded pointer is resolvable by the worker. Hand the absolute anyway: unambiguous, free, and the only option for a handed file.)
  - **Enforced, not just documented:** `node scripts/check-pointers.mjs` resolves every pointer of the first two forms, and the shell form, across `skills/`, `agents/`, `CLAUDE.md` and `README.md`, at exact on-disk case. A new addendum whose pointer is wrong goes red there rather than at the moment some model tries to follow it.
