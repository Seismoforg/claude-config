...
- New or edited step/branch in a skill's WORKFLOW → trace the skill's own activation/entry section to it. An entry that jumps past it leaves it dead: it reads as active, never runs, and every rule inside it is fiction. Applies to a step you only EDITED too — the entry may have routed around it from the start.
