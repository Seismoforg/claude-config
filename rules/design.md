...
- **`MOTION_INTENSITY: 6`** - 1 = Static, 10 = Cinematic / Physics
  ...
  - **PRECEDENCE (mandatory):** on landing / portfolio / marketing surfaces, MOTION_INTENSITY
    overrides the timing figures in `web.md` §4. Those figures are the default only when
    this addendum is out of scope. Mirror half: `web.md` §4.
...
> Install command per system → `design/install-commands.md`. Official doc link to ground the
> choice → `design/canonical-sources.md`. Load either when you pick a system above — the
...
> Font pools, palette alternatives and the banned hex list → `design/design-directives.md`.
...
LLMs default to "static successful state only." `web.md` §6 owns loading/empty/error/success — never ship the happy path alone. Design-specific riders:
...
- **BUTTON CONTRAST CHECK (mandatory, a11y):** Before shipping any button, verify the button text is readable against the button background. White button + white text, `bg-white` CTA with `text-white` label, transparent button against the page background with no border → all banned. Audit every CTA against `web.md` §2 — it owns the contrast thresholds; never restate them here (18px is NOT "large" under WCAG). Same rule applies to ghost buttons over photographic backgrounds (use a backdrop, scrim, or stroke).
...
- **FORM CONTRAST CHECK (mandatory, a11y):** Form inputs, placeholder text, focus rings, helper text, and error text all pass WCAG AA contrast against the section background. Light placeholders on a near-white form, white form on white page section, form labels below the AA floor → all banned. Thresholds: `web.md` §2. Audit every form before shipping.
...
- **HERO TOP PADDING CAP (mandatory):** Hero top padding max `pt-24` (≈6rem) at desktop. If your hero needs more breathing room, increase font scale or asset size, not top padding.
...
- **EYEBROW RESTRAINT (mandatory — the #1 violated rule in production tests).** An eyebrow = the small uppercase wide-tracking label above a section headline (`FOUR COLORWAYS`, `SELECTED WORK`); CSS signature `text-[11px] uppercase tracking-[0.18em]` or `font-mono text-[10.5px] uppercase tracking-[0.22em]`. **Max 1 eyebrow per 3 sections** (hero counts as 1; 9 sections → at most 3). Section A has one → the next 2 cannot. `preflight.mjs` counts `uppercase tracking` and fails when count > ceil(sections/3). **Instead of an eyebrow: drop it.** The headline alone is enough; the section's position already categorises it.
...
Landing pages and portfolios are **visual products**.

**Priority order for visual assets:**
1. **Image-generation tool first.** If ANY image-gen tool is available in the environment (`generate_image`, MCP image tool, IDE-integrated gen, OpenAI image tools, etc.) you MUST use it to create section-specific assets: hero photography, product shots, texture backgrounds, mood images. Generate at the right aspect ratio for the section.
...
**Even minimalist sites need real images.** Even an editorial Linear-style site needs at least 2-3 real images (hero, one product/lifestyle shot, one supporting image). Generate B&W minimalist photography if the brief is restrained; do not skip images entirely because the dial is low.
...
Cut ruthlessly: landing pages live on the first impression, not the full read.
...
- **Spec sheets (the cookware/hardware/apparel pattern):** a long spec table with `border-b` on every row is BANNED. Alternatives → `design/design-directives.md` §4.9 (2-col card grid · scroll-snap pills · grouped chunks · featured-vs-rest disclosure).
- **COPY SELF-AUDIT (mandatory before ship):** re-read EVERY visible string (headlines, subheads, eyebrows, button labels, body, captions, alt text, footer, errors). Rewrite any that is grammatically broken, has unclear referents, sounds like AI hallucination (cute-but-wrong wordplay, forced metaphors), or reads like an LLM trying to sound thoughtful (passive-aggressive humility, fake-craftsman labels, mock-poetic micro-meta). Unsure a string makes sense → replace with a plain functional sentence. Examples → reference.
...
- **MOTION MUST BE MOTIVATED (mandatory).** Every animation answers "what does this communicate?" — valid: hierarchy · storytelling (sequence matching a narrative) · feedback · state transition. Invalid: "it looked cool". Can't articulate the reason in one sentence → drop the animation.
- **MARQUEE MAX-ONE-PER-PAGE (mandatory).** Horizontal scrolling text marquees (endless logos, sideways manifesto, kinetic word strip) at most ONCE per page. `preflight.mjs` counts them. Pick the one section the marquee actually serves; the rest get a different layout.
- **GSAP Sticky-Stack / Horizontal-Pan:** a scroll card-stack must be a REAL sticky-stack, not a sequential reveal list. Canonical skeletons → `design/motion-skeletons.md` (load only when implementing). Common failure for both: the trigger fires before the section pins, so the user sees half a slide. Fix: `start: "top top"` (never `"top center"`/`"top 80%"`), pin the wrapper, scrub the inner track.
...
`web.md` §4 owns the rule; it composes on every web surface. Design-specific rider: at
...
## 6.D Core Web Vitals
`web.md` §3 owns the targets and the optimisation rules. Design-specific rider: the hero image
...
Both light and dark fully styled and token-correct in code. Eyeballing each rendered mode → LIVE VERIFICATION in `web.md`.
...
Resources & Components, §9.F Production-Test Tells) → `design/ai-tells.md`. Load it when
...
Pattern names (heroes, nav, grids, cards, scroll, galleries, typography, micro-interactions, animation-library choice) → `design/pattern-vocabulary.md`. A vocabulary, not a library — load when naming or planning a layout/motion pattern.

**Block Library** = the forward contract for real implementations (props, motion specs, code sketches) in `rules/design/blocks/` — **not yet populated, so there is nothing to load today.** Authoring schema → `design/block-library-schema.md`, load only when actually adding a block.
...
## 11.B-11.F Redesign detail → `design/redesign-protocol.md`
...
Two passes, in order, before outputting code. This is the last filter. **NOT OPTIONAL.**

**1. Mechanical — run the script, never eyeball these.**
```
node ~/.claude/rules/scripts/preflight.mjs <changed files or dir>
```
...
**2. Judgment — walk every box below against the WHOLE page**, not the slice you last edited. The
script cannot see hero fit, zigzag cap, layout-family repetition, copy quality, div-fake-screenshots
or bento rhythm; those are yours.

You wrote the page, so you are primed not to see your own tells. Re-read the rendered output, not
your source. Fix everything the script and this walk returned. Then every box must tick.
...
- [ ] **Core Web Vitals** — meet `web.md` §3 thresholds (§6.D)?
...
- `design/redesign-protocol.md` — §11.B-11.F audit/preservation/levers, only in a redesign mode
- `design/design-directives.md` — §4 font pools, palette alternatives + banned hexes, spec-sheet + copy-audit detail
- `design/ai-tells.md` — §9.A-9.F banned-pattern catalog + examples
- `design/pattern-vocabulary.md` — §10 hero/nav/grid/card/motion naming
- `design/motion-skeletons.md` — §5 sticky-stack / horizontal-pan / scroll-reveal code
- `design/install-commands.md` — §2 install commands per design system
- `design/canonical-sources.md` — §2 canonical official doc links per system
- `design/liquid-glass.md` — §5 Apple Liquid Glass honest web approximation
- `design/block-library-schema.md` — §10 authoring schema, only when adding a block

Factually uncertain — is a package still official and maintained, does an API still work as
documented → search the web, never guess. Brief ambiguity stays under §0.C: ask one question.
