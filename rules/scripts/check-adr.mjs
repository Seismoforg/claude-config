...
//
// The defect this exists for is a READING failure, not a writing one. `rules/documentation.md ADR section`
...
//     `rules/documentation.md` owns those and no script here enforces them.
...
// The three `rules/documentation.md` allows, and nothing else.
...
for (const { file, id, path } of corpus) {
  ...
  if (!STATUSES.has(status)) {
    violations.push(`${rel}  unknown-status  status "${status}" is not one of ${[...STATUSES].join(', ')} (rules/documentation.md)`);
    continue;
  }
