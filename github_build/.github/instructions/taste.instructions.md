...
> Install command per system → `.github/instructions/taste/reference/install-commands.md`. Official doc link to ground the
> choice → `.github/instructions/taste/reference/canonical-sources.md`. Load either when you pick a system above — the
...
> Font pools, palette alternatives and the banned hex list → `.github/instructions/taste/reference/design-directives.md`.
...
* **PREMIUM-CONSUMER PALETTE BAN (mandatory, second-most-recurring AI-tell):** for premium-consumer briefs (cookware, wellness, artisan, luxury, heritage craft, DTC home goods) the LLM default is warm beige/cream + brass/clay/oxblood/ochre + espresso text. BANNED as the default reach — the brand becomes invisible. Exact hexes + 7 rotation alternatives → `.github/instructions/taste/reference/design-directives.md`; `preflight.mjs` greps the hexes. **Rotation rule:** never ship the same warm-craft palette twice in a row. **Override:** only when the brief names those colours, or the identity is genuinely vintage/artisan/warm-craft AND you can articulate why it fits THIS brand.
...
* **Spec sheets (the cookware/hardware/apparel pattern):** a long spec table with `border-b` on every row is BANNED. Alternatives → `.github/instructions/taste/reference/design-directives.md` §4.9 (2-col card grid · scroll-snap pills · grouped chunks · featured-vs-rest disclosure).
...
* **GSAP Sticky-Stack / Horizontal-Pan:** a scroll card-stack must be a REAL sticky-stack, not a sequential reveal list. Canonical skeletons → `.github/instructions/taste/reference/motion-skeletons.md` (load only when implementing). Common failure for both: the trigger fires before the section pins, so the user sees half a slide. Fix: `start: "top top"` (never `"top center"`/`"top 80%"`), pin the wrapper, scrub the inner track.
...
Both light and dark fully styled and token-correct in code. Eyeballing each rendered mode → LIVE VERIFICATION in `.github/blocks.md`.
...
Resources & Components, §9.F Production-Test Tells) → `.github/instructions/taste/reference/ai-tells.md`. Load it when
...
Pattern names (heroes, nav, grids, cards, scroll, galleries, typography, micro-interactions, animation-library choice) → `.github/instructions/taste/reference/pattern-vocabulary.md`. A vocabulary, not a library — load when naming or planning a layout/motion pattern.

**Block Library** = the forward contract for real implementations (props, motion specs, code sketches) in `skills/taste/blocks/` — **not yet populated, so there is nothing to load today.** Authoring schema → `.github/instructions/taste/reference/block-library-schema.md`, load only when actually adding a block.
...
## 11.B-11.F Redesign detail → `.github/instructions/taste/reference/redesign-protocol.md`
...
```
node .github/scripts/preflight.mjs <changed files or dir>
```
...
`.github/instructions/taste/reference/ai-tells.md`. You wrote the page, so you are primed not to see your own tells: self-review
...
- `.github/instructions/taste/reference/redesign-protocol.md` — §11.B-11.F audit/preservation/levers, only in a redesign mode
- `.github/instructions/taste/reference/design-directives.md` — §4 font pools, palette alternatives + banned hexes, spec-sheet + copy-audit detail
- `.github/instructions/taste/reference/ai-tells.md` — §9.A-9.F banned-pattern catalog + examples
- `.github/instructions/taste/reference/pattern-vocabulary.md` — §10 hero/nav/grid/card/motion naming
- `.github/instructions/taste/reference/motion-skeletons.md` — §5 sticky-stack / horizontal-pan / scroll-reveal code
- `.github/instructions/taste/reference/install-commands.md` — §2 install commands per design system
- `.github/instructions/taste/reference/canonical-sources.md` — §2 canonical official doc links per system
- `.github/instructions/taste/reference/liquid-glass.md` — §5 Apple Liquid Glass honest web approximation
- `.github/instructions/taste/reference/block-library-schema.md` — §10 authoring schema, only when adding a block

See `.github/blocks.md` for WHEN UNCERTAIN (factual uncertainty — is a package still official/maintained, does an API still work as documented; brief ambiguity stays under §0.C, ask one question) / AFTER THE TASK / LANGUAGE.
