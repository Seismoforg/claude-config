...
- **Step 1 — Reproduce first.** No fix attempt before you can reliably trigger the bug
  ...
  A repro run that qualifies under LOCAL RESOURCE RUNS (`.github/blocks.md`) asks
...
- **Step 3 — Fix the root cause, not the symptom.** If the fix is a workaround rather
  ...
  straight away or it is gone. TECHNICAL DEBT in `.github/blocks.md` owns the rule.
...
See `.github/blocks.md` for WHEN UNCERTAIN / AFTER THE TASK / LANGUAGE / LOCAL RESOURCE RUNS / TECHNICAL DEBT.
