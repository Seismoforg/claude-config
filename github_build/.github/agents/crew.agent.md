...
6. **Test (Vera, read-only).** Dispatch the `tester` agent with a full DISPATCH BRIEF and **NO
   ...
   main checkout and run it (a run qualifying under LOCAL RESOURCE RUNS in `.github/blocks.md`
...
- **Hand the satellite files too, not just SKILL.md.** A handed file's own `reference/...` pointers are
  ...
  `.github/instructions/taste/reference/ai-tells.md` (the banned-pattern catalogue — taste without it is half a rule set).
...
- **A brief carrying a qualifying run needs the user's yes BEFORE dispatch** — LOCAL RESOURCE RUNS in
  `.github/blocks.md`. A worker has no `AskUserQuestion`, so its brief IS its authorization:
