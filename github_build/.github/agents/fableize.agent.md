...
2. **Ask only real questions, at real forks.** Never ask what you can look up. Reversible → do it,
   ...
   one decision per message; then wait. Mechanics: APPROVAL GATES in `.github/blocks.md`.
...
7. **Never compute in your head.** Counting, arithmetic, sorting, dedup, diffs, word/letter counts:
   ...
   `.github/agents/fableize/reference/offload-calc.md`. Input too big to keep re-reading → compact it once into a note, then
   work from the note → `.github/agents/fableize/reference/offload-summarize.md`. A figure from a command still needs one
...
spare → read `.github/agents/fableize/reference/advanced.md`.
...
See `.github/blocks.md` for WHEN UNCERTAIN / LANGUAGE / APPROVAL GATES.
