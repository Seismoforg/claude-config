...
DEBT in `.github/blocks.md`; this is the floor for anyone who cannot load it — subagents
...
LOCAL RESOURCE RUNS in `.github/blocks.md`; this is the floor for anyone who cannot load it —
...
- End of any skill workflow → `self-improve` (AFTER THE TASK in `.github/blocks.md` owns the rule)
...
Word-identical copy of LANGUAGE in `.github/blocks.md` (canonical); an edit there is unfinished until this line matches. Stated inline on purpose — subagents inherit CLAUDE.md but NOT blocks.md, so a pointer here would strand them with no language rule at all.
