...
- **analysis** (default, absent = this) — read-only workers: `audit-scout`, `security-auditor`,
  `standards-reviewer`. Hold no write tools; they return findings or code as TEXT.
...
**A role that must SEE an earlier worker's output cannot be an executor.** A worktree is cut from a
base that may predate the very work it was sent to check, so it cannot carry one worker's output to
the next. Build such a role as ANALYSIS: it returns TEXT and the dispatcher applies it.
`standards-reviewer` is that shape — it reads a diff the dispatcher names and writes nothing. Full
reasoning and the measurement behind it:
[ADR 0001](../docs/adr/0001-tester-is-read-only-not-an-executor.md). Its own subject, the `tester`
agent, was retired with the crew skill; the principle it establishes was not.
...
- No agent here dispatches another — **repo POLICY, not a harness limit.** The harness allows nesting
  ...
  main-loop-only — see `feature` step 5, which cuts a milestone's tasks into file-disjoint sets and
  dispatches `dev` workers from the main loop.
- Skills are NOT auto-discovered inside a subagent. Each agent preloads what it needs via the
  ...
  over as an absolute path (`feature/reference/dispatch.md` §3 owns that). An agent's cwd is the target repo, so
...
- **executor** → a body line documenting its worktree isolation (`unbriefed-executor`). The worktree
  is a dispatch-time flag (`isolation: worktree`) the check cannot set, so the dispatcher must pass it.
